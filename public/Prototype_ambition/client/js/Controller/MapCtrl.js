...
app.controller("MapCtrl", function ($rootScope, $scope, MyService,PostService, $element, $compile, $http, LoginService) {
    var map, infoWindow;
    var markers = [];
    var dislikedEvents = [];
    ...
    function setMarker(map, position, title, content) {
        ...
        var markerOptions = {
            ...
            title: title,
            icon: '/Prototype_ambition/client/img/red-dot.png'
        };

        if (title == 'You are here') {
            markerOptions.icon = '/Prototype_ambition/client/img/blue_dot.png';
    ...
    function printOnMap(oArgs) {
        ...
        if (filterAppliedOfferingType() ||
            filterAppliedFoodType() ||
            filterAppliedPlace() ||
            ...
            filterAppliedToDate())
        {
            ...
            filterApplied = true;
            if (filterAppliedOfferingType()) oArgs.offeringType = $scope.filter.offeringType;
            if (filterAppliedFoodType()) oArgs.foodType = $scope.filter.foodType;
            if (filterAppliedPlace())
            {
                oArgs.place_x = $scope.filter.place_x;
                oArgs.place_y = $scope.filter.place_y;
            }
        ...
        PostService.getPosts($scope.filter,
            function (events)
            {
                if (events == null)
                {$scope.loading = false;
                ...
                for (var d in events) {
                    ...
                    //if (!userDislikedVenue(event.venue_id)) {
                        ...
                        setMarker(map, new google.maps.LatLng(event.place_x, event.place_y), event.title, compiled[0]);
                    //}
                }
                $scope.loading = false;
            });
        ...
        if (spinner == undefined) {
            spinner = angular.element("<center><img src='/Prototype_ambition/client/img/gps.gif' ng-show='loading' id='spinner'></center>");
    ...
    function filterAppliedOfferingType() {
        if ($scope.filter != null) {
            if ($scope.filter.offeringType == null || $scope.filter.offeringType == '') return false;
            else return true;
        }
        return false;
    }

    function filterAppliedFoodType() {
        if ($scope.filter != null) {
            if ($scope.filter.foodType == null || $scope.filter.foodType == '') return false;
            else return true;
        }
        return false;
    }
    ...
    function filterAppliedPlace() {
        if ($scope.filter != null) {
            if ($scope.filter.place_x == null || $scope.filter.place_x == '' ||
                $scope.filter.place_y == null || $scope.filter.place_y == '') return false;
            else return true;
        }
        return false;
    }
